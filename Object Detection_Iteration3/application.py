...
from load_model import preprocessing, predict_binary, predict_tomato_leaf_or_not # Funtion to invoke model
...
class GetObjectPrediction(Resource):
    ...
    def post(self):
        ...
        if len(imgstring) != 0: 
            ...
            output_image, count = yolov3_tomato_detection([filename])
            ...
            os.remove(filename) 
            # sending response to app with status code
            return make_response(jsonify({ "image": aws_file_path, "count": count }), 200)
        else:
            # If not contained data sending error stating 'Select the Image'
            return make_response(jsonify({"Error": "Please select the Image"}), 400)
